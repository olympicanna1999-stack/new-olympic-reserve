# 🏆 Олимпийский резерв - НОВОЕ ПРИЛОЖЕНИЕ v5.0
# ДЛЯ НОВОГО РЕПОЗИТОРИЯ - БЕЗ ПРОБЛЕМ!
...
# САМОЕ ВАЖНОЕ: правильное имя БД!
DB_NAME = 'app.db'
...
def show_athletes():
    ...
    # Таблица
    display_df = df[['full_name', 'gender', 'age', 'sport', 'region', 'vo2_max_ml_kg_min']].copy()
    display_df.columns = ['ФИО', 'Пол', 'Возраст', 'Вид спорта', 'Регион', 'VO₂max']
    
    st.dataframe(display_df, use_container_width=True, hide_index=True)
    st.write(f"**Всего:** {len(df)} спортсменов")
...
def show_results():
    ...
    display_results = athlete_results[['competition_date', 'distance_km', 'finish_position', 'medal']].head(20)
