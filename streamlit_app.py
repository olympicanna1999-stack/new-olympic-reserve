# 🏆 Олимпийский резерв - ИСПРАВЛЕННОЕ ПРИЛОЖЕНИЕ v5.2
# Исправлена ошибка "Cannot operate on a closed database"
...
def get_db_connection():
    """Подключение к БД - ОСТАЕТСЯ ОТКРЫТЫМ"""
    try:
...
def load_athletes():
    """Загрузить спортсменов - БЕЗ закрытия БД"""
    try:
        conn = get_db_connection()
        if conn is None:
            return pd.DataFrame()
        df = pd.read_sql('SELECT * FROM athletes ORDER BY full_name', conn)
        # ИСПРАВЛЕНИЕ: НЕ закрываем БД!
        return df
    except Exception as e:
...
def load_competition_results():
    """Загрузить результаты - БЕЗ закрытия БД"""
    try:
        conn = get_db_connection()
        if conn is None:
            return pd.DataFrame()
...
def show_results():
    ...
    if df.empty:
        st.warning("⚠️ Спортсмены не загружены")
        return
    
    if df_results.empty:
        st.warning("⚠️ Результаты не загружены")
        return
    ...
    if athlete_results.empty:
        st.info("ℹ️ Результатов нет для этого спортсмена")
        return
    ...
    st.subheader(f"Результаты {selected} ({len(athlete_results)} соревнований)")
    ...
    with col1:
        if not athlete_results.empty and len(athlete_results) > 1:
